...
class LaserTankMap:
    ...
    def new_apply_move(self, r, REWARDS, EXIT_STATE):
        if (self.player_x,self.player_y) in REWARDS:
            return REWARDS[(self.player_x,self.player_y)],EXIT_STATE[0],EXIT_STATE[1]

        if (self.player_x,self.player_y) == EXIT_STATE:
            return 0, self.player_x, self.player_y
        ...
        if self.player_heading == self.UP:
            ...
            if next_y < 1 or next_x < 1 or next_x >= self.x_size - 1:
                return 0, self.player_x, self.player_y
        elif self.player_heading == self.DOWN:
            ...
            if next_y >= self.y_size - 1 or next_x > 1 or next_x <= self.x_size - 1:
                return 0, self.player_x, self.player_y
        elif self.player_heading == self.LEFT:
            ...
            if next_x < 1 or next_y < 1 or next_y >= self.y_size - 1:
                return 0, self.player_x, self.player_y
        else:
            if r < self.t_success_prob:
                next_y = self.player_y
            ...
            if next_x >= self.x_size - 1 or next_y < 1 or next_y >= self.y_size - 1:
                return 0, self.player_x, self.player_y

        if self.cell_is_blocked(next_y, next_x):
            return self.collision_cost, self.player_x, self.player_y
            # check for game over conditions
        if self.cell_is_game_over(next_y, next_x):
            return self.game_over_cost, self.player_x, self.player_y

        if self.grid_data[next_y][next_x] == self.FLAG_SYMBOL:
            return self.goal_reward, next_x, next_y
        else:
            return self.move_cost, next_x, next_y

    def new_turning(self, move):
